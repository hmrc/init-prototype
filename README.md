...
1. Clone from Github to a local directory any repositories in the HMRC organisation that are named the same as a 
   deployed Heroku prototype
1. Produce a report of which deployed Heroku prototypes have a matching Github repository, and whether that repository 
   contains a `package-lock.json` file.
   
The output will be saved to `package-local-analysis.txt`.
